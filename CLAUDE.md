...
- **참조 문서 246개(약 5,928벡터)**: 232개 HS 표준해석 지침 파싱본 + 기존 14개 문서 적재 완료
- **Clarification 흐름 미구현**: 충돌 시 배너·A/B 비교표 표시까지 구현, 에이전트 추가 질문(2턴) 미구현
- **UNI-PASS API 미연동**: 정적 세율 테이블 9개 류(28/38/39/69/76/84/85/90류 등) 커버, 실시간 조회 불가
...
**SC-003: 재질 변경 재분류 (세라믹 엣지 링):**
```
품목명: 웨이퍼 엣지 링 (Edge Ring)
재질:   산화알루미늄(Al₂O₃) 세라믹
용도:   플라즈마 식각 장비 내 웨이퍼 가장자리 보호 및 플라즈마 균일 제어
공정:   전공정 Etch
수출입: 수입
기존 HS-Code: 7616999090  ← 알루미늄 소재 시절 코드
유사도 임계값: 0.30
→ 기대:
  - conflict_flag=True, TYPE-2 (76류 기존 ↔ 69류/84류 추천)
  - A/B 비교표 표시: 후보A=7616999090(알루미늄) vs 후보B=6909190000(세라믹)
  - "사내 코드 통일 검토 필요" 경고 메시지
  - 세율 차이 표시: 76류 8% vs 84류 0% (반도체 장비 면세)
  - Fallback=아니오
검증 포인트:
  - 재질 변경만으로 류가 바뀌는 케이스에서 TYPE-2 정상 감지
  - 세율 변화(비용 절감 가능성) 자동 안내 여부
```
